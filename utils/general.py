...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, fname='precision-recall_curve.png'):
    ...
    # Sort by objectness
    i = np.argsort(-conf)  # index ,数据结构为list,置信度从大到小排序
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_l == 0:
            ...
            fpc = (1 - tp[i]).cumsum(0)  # cumsum(0) 最外层累加
            tpc = tp[i].cumsum(0)
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x = x[xc[xi]]  # confidence  去除掉conf值没达到阈值的
        ...
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T  # 返回非零元素坐标（i,j）
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    img = cv2ImgAddText(img, label, c1[0], c1[1] - 2, (color[0], color[1], color[2]))
    ...
    if label:
        ...
        # cv2ImgAddText(img, label, c1[0], c1[1] - 2, (color[0], color[1], color[2]))
